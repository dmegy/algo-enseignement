...
%le produit d'ordres totaux n'est pas total.
% le produit de bon ordres n'est pas un bon ordre
...
def soustr(m,n):
	if n == 0: return m
	if m == 0: return 0
	return soustr(m-1,n-1)
```
vu sur https://www.lip6.fr/Christine.Tasson/doc/cours/M1sem/TD1.pdf
Exercices : ordre lexicographique, ordre multiset et application à la terminaison du parcours des graphes ?

- algo aléatoire (d'après Dowek) (garder pour examen?)(non, donner tt de suite)
source : https://math.univ-lyon1.fr/irem/IMG/pdf/terminaison_avec_solutions.pdf page 4
```python
# rand() est une fonction retournant un nombre entier altéoire non borné !!
def f(n,p):
	if p != 0:
		return f(n,p-1)
	elif n != 0:
...
Si rand() est bornée a priori par $M$, alprs il y a au plus $p + (n-1)*M$ appels.
Mais même si rand() n'est pas bornée, ça termine car le couple décroit strictement pour l'ordre lexicographique.

- Petites suites de Goodstein (d'après Dowek) (garder pour exam ou TP)
source : toujours https://math.univ-lyon1.fr/irem/IMG/pdf/terminaison_avec_solutions.pdf
On écrit un nombre en base deux, 
on interprete en base trois, on enlève un et on considère l'écriture en base 3.
on interprete comme une écriture en base 4. On enlève un, on écrit en base quatre
etc...
Montrer... que la suite stationne à zéro !!

Preuve : la suite des chiffres (dans des bases qui augmentent, et quitte à mettre des zéros devant) est strictement décroissante dans N^r, si r est la quantité initiale de chiffres en base deux.
